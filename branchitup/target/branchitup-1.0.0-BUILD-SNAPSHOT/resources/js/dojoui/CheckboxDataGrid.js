/* Dojo 1.4 */
if(!dojo._hasResource["dojoui.CheckboxDataGrid"]){ //_hasResource checks added by build. Do not use _hasResource directly in your code.
	dojo._hasResource["dojoui.CheckboxDataGrid"] = true;
	dojo.provide("dojoui.CheckboxDataGrid");

	dojo.require("dojox.grid.DataGrid");
	dojo.require("dojo.data.ItemFileWriteStore");
	dojo.require("dojo.data.ItemFileReadStore");

	dojo.declare("dojoui.CheckboxDataGrid", dojox.grid.DataGrid, {
...
//				this.store = new dojo.data.ItemFileWriteStore({data: this._gridData, typeMap: this.typeMap});
			...
			div.appendChild(this._createHeaderCheckbox());
			con.appendChild(div);
			
//			dojo.connect(this,"onSelectionChanged",function(e){
//				console.log("on selection changed");
//				console.log(e);
//			});
			
			dojo.connect(this,"onDeselected",function(inRowIndex){
				var selectedCount = self.selection.getSelectedCount();
				if(selectedCount <= 0){
...
//				this.store.save();
		...
		removeByIds: function(ids){
			this.selection.clear();
			for(var i in ids){
				var itemToRemove = this.itemsMap[ids[i]];
				this.store.deleteItem(itemToRemove);
...
//			this._lastFilterQuery = query;
//			//sets this.query
//			this.setQuery(query,{ignoreCase:true, deep: true, cache: true});
//			
//		},
//		refilter: function(){
//			if(this._lastFilterQuery){
//				this.filter(this._lastFilterQuery);
//			}
//		},
...
//			this.selection.clear();
		...
		resort: function(){
			...
			//refresh
			//clear
//			this.updateRowCount(0); //causes the table to flash, and sometimes to blank
			this._by_idty = {};
			this._by_idx = [];
			this._pages = [];
...
//			console.log("update items:::::"+items.length);
			for(var i in items){
				var newItem = items[i];
				var oldItem = this.itemsMap[newItem[this.identifier]];
				for(var k in newItem){
					if(k != this.identifier){
						if(typeof newItem[k]._type != 'undefined'){
							...
							this.store.setValue(oldItem,k,v);
						}
						else{
							this.store.setValue(oldItem,k,newItem[k]);
						}
					}
				}
			}
...
//			this._setQuery({},{ignoreCase:true, deep: false, cache: false}); //clears the filter
			for(var i in items){
				this.itemsMap[items[i][this.identifier]] = this.store.newItem(items[i]);
			}
//			this.store.save(); //causes new unrelated records to appear in the table when filter is activated
			this.resort();
		},
		scheduleSort: function(){
			if(this.timer != null){
				clearTimeout(this.timer);
				this.timer = null;
			}
			this.timer = setTimeout(dojo.hitch(this,function(){this.timer = null; this.sort(); }),50);
		},
		timer: null,
		updateItem: function(item,newItem){ //DOES NOT WORK WELL
//			this._setQuery({},{ignoreCase:true, deep: false, cache: false});
			for(var k in newItem){
				if(k != this.identifier){
					if(typeof newItem[k]._type != 'undefined'){
						var v = this.typeMap[newItem[k]._type].deserialize(newItem[k]._value);
