...
if (!dojo._hasResource["dojoui.locale"]) {
	dojo._hasResource["dojoui.locale"] = true;
	dojo.provide("dojoui.locale");
	...
	( function() {
		function _1(_2, _3, _4, _5) {
			return _5.replace(/([a-z])\1*/ig, function(_6) {
				var s, _7, c = _6.charAt(0), l = _6.length, _8 = [ "abbr",
						"wide", "narrow" ];
				switch (c) {
				case "G":
					s = _3[(l < 4) ? "eraAbbr" : "eraNames"][_2.getUTCFullYear() < 0 ? 0 : 1];
					break;
				case "y":
					s = _2.getUTCFullYear();
					switch (l) {
				...
				case "Q":
				case "q":
					s = Math.ceil((_2.getUTCMonth() + 1) / 3);
					_7 = true;
					break;
				case "M":
					var m = _2.getUTCMonth();
					if (l < 3) {
						s = m + 1;
						_7 = true;
					} else {
				...
				case "E":
					...
					break;
				case "a":
					var _c = (_2.getUTCHours() < 12) ? "am" : "pm";
					s = _3[_c];
					break;
				case "h":
				case "H":
				...
				case "s":
					...
					break;
				case "S":
					s = Math.round(_2.getUTCMilliseconds()
							* Math.pow(10, l - 3));
					_7 = true;
					break;
				case "v":
				...
				case "Z":
					...
					var tz = [ (_d <= 0 ? "+" : "-"),
							dojo.string.pad(Math.floor(Math.abs(_d) / 60), 2),
							dojo.string.pad(Math.abs(_d) % 60, 2) ];
					if (l == 4) {
						tz.splice(0, 0, "GMT");
						tz.splice(3, 0, ":");
					}
		...
		dojo.date.locale.format = function(_11, _12) {
			_12 = _12 || {};
			var _13 = dojo.i18n.normalizeLocale(_12.locale), _14 = _12.formatLength
					|| "short", _15 = dojo.date.locale._getGregorianBundle(_13), str = [], _16 = dojo
					.hitch(this, _1, _11, _15, _12);
			...
			return str.length == 1 ? str[0] : _15["dateTimeFormat-" + _14]
					.replace(/\{(\d+)\}/g, function(_19, key) {
						return str[key];
					});
		};
		...
		dojo.date.locale._parseInfo = function(_1b) {
			_1b = _1b || {};
			var _1c = dojo.i18n.normalizeLocale(_1b.locale), _1d = dojo.date.locale
					._getGregorianBundle(_1c), _1e = _1b.formatLength
					|| "short", _1f = _1b.datePattern
					|| _1d["dateFormat-" + _1e], _20 = _1b.timePattern
					|| _1d["timeFormat-" + _1e], _21;
			if (_1b.selector == "date") {
				...
				if (_1b.selector == "time") {
					...
					_21 = _1d["dateTimeFormat-" + _1e].replace(/\{(\d+)\}/g,
							function(_22, key) {
								return [ _20, _1f ][key];
							});
				}
		...
		dojo.date.locale.parse = function(_25, _26) {
			var _27 = dojo.date.locale._parseInfo(_26), _28 = _27.tokens, _29 = _27.bundle, re = new RegExp(
					"^" + _27.regexp + "$", _27.strict ? "" : "i"), _2a = re.exec(_25);
			if (!_2a) {
				return null;
			}
			var _2b = [ "abbr", "wide", "narrow" ], _2c = [ 1970, 0, 1, 0, 0,
					0, 0 ], _2d = "", _2e = dojo.every(
							_2a,
							function(v, i) {
								if (!i) {
									return true;
								...
								case "y":
									if (l != 2 && _26.strict) {
										...
										if (v < 100) {
											v = Number(v);
											var _30 = ""
													+ new Date().getUTCFullYear(), _31 = _30
													.substring(0, 2) * 100, _32 = Math
													.min(Number(_30.substring(
															2, 4)) + 20, 99), num = (v < _32) ? _31
													+ v
													: _31 - 100 + v;
											_2c[0] = num;
										} else {
								...
								case "E":
								case "e":
									var _34 = _29["days-format-" + _2b[l - 3]]
											.concat();
									if (!_26.strict) {
										v = v.toLowerCase();
			...
			var _37 = new Date(_2c[0], _2c[1], _2c[2], _2c[3], _2c[4], _2c[5],
					_2c[6]);
			if (_26.strict) {
				_37.setFullYear(_2c[0]);
			}
			var _38 = _28.join(""), _39 = _38.indexOf("d") != -1, _3a = _38.indexOf("M") != -1;
			if (!_2e || (_3a && _37.getUTCMonth() > _2c[1]) || (_39 && _37.getUTCDate() > _2c[2])) {
				return null;
			}
			if ((_3a && _37.getUTCMonth() < _2c[1]) || (_39 && _37.getUTCDate() < _2c[2])) {
	...
	dojo.date.locale.isWeekend = function(_56, _57) {
		var _58 = dojo.cldr.supplemental.getWeekend(_57), day = (_56 || new Date()).getUTCDay();
	...
	dojo.date.locale._getWeekOfYear = function(_5a, _5b) {
		...
		var _5c = new Date(_5a.getYear(), 0, 1).getUTCDay(), adj = (_5c - _5b + 7) % 7, _5d = Math.floor((dojo.date.locale._getDayOfYear(_5a) + adj - 1) / 7);
		if (_5c == _5b) {
			_5d++;
		}
		return _5d;
